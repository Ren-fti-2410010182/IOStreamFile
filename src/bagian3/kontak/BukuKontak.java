...
public class BukuKontak {
    ...
    public void hapusKontak(String nama) {
        // Menghapus semua kontak yang namanya cocok dengan input
        boolean berhasil = daftar.removeIf(k -> k.getNama().equalsIgnoreCase(nama));

        if (berhasil) {
            System.out.println("Kontak '" + nama + "' berhasil dihapus.");
            // Setelah dihapus, simpan perubahan ke berkas agar sinkron
            simpanKeBerkas();
        } else {
            System.out.println("Kontak dengan nama '" + nama + "' tidak ditemukan.");
        }
